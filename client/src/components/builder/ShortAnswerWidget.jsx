import {
    ...
    AlignCenter, ArrowDownUp, Check, X, Maximize2, Minimize2, FileText, Globe
} from 'lucide-react';
import toast from 'react-hot-toast';
import { useState, useRef, useEffect } from 'react';

const ShortAnswerWidget = ({ onDelete, onDuplicate, onUpdate, initialText = "" }) => {
    ...
    const handleAiGeneration = () => {
        ...
        if (editorRef.current) {
            ...
            // Simulate AI typing
            editorRef.current.innerText = "Generating question...";
        }

        setTimeout(() => {
            if (editorRef.current) {
                editorRef.current.innerText = "Describe the impact of artificial intelligence on modern education systems.";
            }
    ...
    const handleMicClick = () => {
        ...
        recognition.onresult = (event) => {
            ...
            const processedText = processMathSpeech(transcript);
            ...
            if (editorRef.current) {
                ...
                if (!success) {
                    editorRef.current.innerText += (editorRef.current.innerText ? ' ' : '') + processedText;
                }
            }
        };
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden font-sans group hover:shadow-md transition-shadow">
            ...
            {isExpanded && (
                <div className="p-6 animate-fade-in">
                    ...
                    {/* Answer Input */}
                    <div className={`flex items-center justify-between border ${enableAnswer ? 'border-indigo-500 ring-1 ring-indigo-100' : 'border-slate-200'} rounded-lg p-0 bg-white mb-6 focus-within:ring-2 focus-within:ring-indigo-100 transition-all overflow-hidden`}>
                        <input
                            type="text"
                            placeholder="Type your Answer here"
                            className="flex-1 bg-transparent outline-none text-slate-600 text-sm px-4 py-3 disabled:bg-slate-50 disabled:text-slate-400"
                            disabled={!enableAnswer}
                        />
                        <div className="flex items-center gap-3 pr-4 border-l border-slate-100 pl-4 bg-slate-50 h-full py-3">
                            <div
                                className="flex items-center gap-2 cursor-pointer select-none"
                                onClick={() => setEnableAnswer(!enableAnswer)}
                            >
                                <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">Enable it</span>
                                <div className={`w-9 h-5 rounded-full relative transition-colors ${enableAnswer ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                                    <div className={`w-4 h-4 bg-white rounded-full absolute top-0.5 shadow-sm transition-transform ${enableAnswer ? 'left-4.5 translate-x-3.5' : 'left-0.5'}`}></div>
                                </div>
                            </div>
                            <button
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                        <div className="flex items-center gap-4">
                            ...
                            >
                                <span className={`text-xs font-bold ${enableTextStyle ? 'text-indigo-600' : 'text-slate-500'}`}>Enable Text Style</span>
