import {
    ...
    AlignCenter, ArrowDownUp, Check, X, Maximize2, Minimize2, FileText
} from 'lucide-react';
import { useState, useRef, useEffect } from 'react';
import toast from 'react-hot-toast';

const VoiceWidget = ({ onDelete, onDuplicate, onUpdate, initialText = "" }) => {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden font-sans group hover:shadow-md transition-shadow mb-6">
            ...
            <div className="flex items-center justify-between p-3 border-b border-slate-100 bg-white">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-[#6F42C1] rounded-lg text-white shadow-sm shadow-purple-200">
                        <Mic size={18} />
                    </div>
                    <span className="font-bold text-slate-700 text-sm">Voice Rec</span>
                </div>
                ...
                <div className="p-6 animate-fade-in">
                    ...
                    <div className="mb-6 relative group/input">
                        <div
                            ref={editorRef}
                            contentEditable
                            className="w-full text-lg text-slate-700 outline-none bg-transparent py-2 border-b-2 border-transparent focus:border-purple-500 transition-colors empty:before:content-[attr(data-placeholder)] empty:before:text-slate-400 rich-text-editor"
                    ...
                    <div className="mb-6">
                        <input
                            ...
                            className="w-full text-sm text-slate-600 px-4 py-3 bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-100 transition-all"
                        />
                    </div>
                    ...
                    <div className="border-2 border-dashed border-slate-300 rounded-lg bg-slate-50/50 p-8 flex items-center justify-center mb-6 min-h-[120px]">
                        <button className="px-6 py-3 bg-white border border-slate-200 shadow-sm rounded-lg text-slate-700 font-semibold hover:border-purple-300 hover:text-purple-700 hover:shadow-md transition-all flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                        <div className="flex items-center gap-4">
                            ...
                            >
                                <span className={`text-xs font-bold ${enableTextStyle ? 'text-purple-600' : 'text-slate-500'}`}>Enable Text Style</span>
                                <div className={`w-9 h-5 rounded-full relative transition-colors ${enableTextStyle ? 'bg-purple-600' : 'bg-slate-300'}`}>
