...
const MarketersList = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this marketer?')) {
            try {
                ...
                setMarketers(marketers.filter(e => e._id !== id));
    ...
    const handleApplyBulkAction = async () => {
        ...
        if (bulkAction === 'delete') {
            if (window.confirm(`Are you sure you want to delete the ${selectedIds.size} selected marketers?`)) {
                try {
                    await Promise.all(Array.from(selectedIds).map(id => axios.delete(`/api/users/${id}`)));
                    toast.success('Successfully deleted selected marketers');
                    setSelectedIds(new Set());
                    setBulkAction('');
                    fetchData();
                } catch (err) {
                    console.error("Bulk delete error:", err);
                    toast.error('Failed to delete some marketers');
                }
            }
        }
    };
    ...
    return (
        <DashboardLayout role={user?.role || 'Admin'}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Marketers Management</h1>
                    <p className="text-slate-500">Manage platform marketers and track their activity.</p>
                </div>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (isNaN(val)) {
                                    ...
                                    const maxVal = filteredMarketers.length > 5 ? filteredMarketers.length : 5;
                        ...
                        <thead>
                            <tr className="bg-slate-50 border border-slate-200 text-slate-500 text-sm uppercase tracking-wider">
                                <th className="p-4 w-10">
                                    <input
                                        type="checkbox"
                                        checked={paginatedMarketers.length > 0 && selectedIds.size === paginatedMarketers.length}
                                        onChange={() => {
                                            if (selectedIds.size === paginatedMarketers.length) {
                                                setSelectedIds(new Set());
                                            } else {
                                                setSelectedIds(new Set(paginatedMarketers.map(item => item._id)));
                                            }
                                        }}
                                        className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 cursor-pointer accent-indigo-650"
                                    />
                                </th>
                                <th className="p-4 font-semibold whitespace-nowrap">Marketer Name</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                paginatedMarketers.map((marketer) => (
                                    <tr key={marketer._id} className="hover:bg-slate-50 transition-colors group">
                                        <td className="p-4 w-10">
                                            <input
                                                type="checkbox"
                                                checked={selectedIds.has(marketer._id)}
                                                onChange={() => {
                                                    setSelectedIds(prev => {
                                                        const next = new Set(prev);
                                                        if (next.has(marketer._id)) {
                                                            next.delete(marketer._id);
                                                        } else {
                                                            next.add(marketer._id);
                                                        }
                                                        return next;
                                                    });
                                                }}
                                        ...
                                        <td className="p-4 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                ...
                                                >
                                                    {marketer.avatar ? (
                                                        <img src={marketer.avatar} alt={marketer.name} className="w-full h-full object-cover" />
                                                    ) : (
                                                        marketer.name[0]
                                                    )}
                                                </div>
                                        ...
                                        <td className="p-4 whitespace-nowrap text-sm text-slate-650 font-medium">
                                            {marketer.marketerProfile?.assignedCourses?.length > 0 ? (
                                                <TruncatedCell text={marketer.marketerProfile.assignedCourses.map(c => c.name || c).join(', ')} maxLength={20} />
                                            ) : (
                                                <span className="text-slate-400 text-xs">N/A</span>
                                            )}
                                        </td>
                                        <td className="p-4 whitespace-nowrap text-sm text-slate-650 font-medium">
                                            {marketer.marketerProfile?.subjects?.length > 0 ? (
                                                <TruncatedCell text={marketer.marketerProfile.subjects.join(', ')} maxLength={20} />
                                            ) : (
                                                <span className="text-slate-400 text-xs">N/A</span>
                                            )}
                                        </td>
                                        ...
                                        <td className="p-4 whitespace-nowrap">
                                            <button
                                                ...
                                                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${marketer.isActive !== false ? 'bg-emerald-500' : 'bg-slate-200'
                                                    }`}
                                                title={marketer.isActive !== false ? 'Click to Deactivate Account' : 'Click to Activate Account'}
                                            >
                                                <span className="sr-only">Toggle status</span>
                                                <span
                                                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${marketer.isActive !== false ? 'translate-x-5' : 'translate-x-0'
                                                        }`}
                                                />
                                            </button>
                                        </td>
                ...
                {filteredMarketers.length > 0 && (
                    <div className="p-4 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 bg-white">
                        <div className="text-sm font-semibold text-slate-500">
                            ...
                            <span className="text-slate-700">{Math.min(startIndex + itemsPerPage, filteredMarketers.length)}</span> of{' '}
                            <span className="text-slate-700">{filteredMarketers.length}</span> entries
                        </div>
            ...
            <RecycleBinModal
                ...
                title="Marketers Recycle Bin"
                trashUrl="/api/users/trash?role=Marketer"
