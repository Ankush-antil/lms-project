...
const TeacherActivities = () => {
    ...
    const filteredMaterials = useMemo(() => {
        return studyMaterials.filter(mat => {
            const type = getMaterialType(mat);
            return selectedCategoryTab === 'all' || type === selectedCategoryTab;
        });
    }, [studyMaterials, selectedCategoryTab]);

    const renderMaterialSlider = (categoryKey, title, materialsList) => {
        if (!materialsList || materialsList.length === 0) return null;
        const scrollId = `teacher-study-materials-scroll-container-${categoryKey}`;

        return (
            <div className="space-y-2.5 mt-5 first:mt-0">
                <div className="flex items-center gap-2">
                    <span className="h-3 w-1 bg-[#3E3ADD] rounded-full"></span>
                                        ...
                                        <div className="flex items-center justify-between mb-2.5">
                                            ...
                                            {selectedStudent && (!isSaDisabled('student') || saMode() === 'disable') && (
                                                <button
                                                    ...
                                                    className={`p-1.5 rounded-full border shadow-sm transition-all ${isSaDisabled('student')
                                            ...
                                            {[
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap ${isDisabled
                                            ...
                                            <div className="space-y-4">
                                                    ...
                                                    <div className="flex bg-slate-100/80 p-0.5 rounded-xl gap-0.5 border border-slate-200/50">
                                                        {[
                                                            ...
                                                            <button
                                                                ...
                                                                className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all ${selectedCategoryTab === tab.id
                                                                        ? 'bg-[#3E3ADD] text-white shadow-sm'
                                                ...
                                                {loadingMaterials ? (
                                                    ...
                                                    <div className="space-y-6">
                                                        {renderMaterialSlider('video', 'Videos', studyMaterials.filter(m => getMaterialType(m) === 'video'))}
                                                        {renderMaterialSlider('audio', 'Audios', studyMaterials.filter(m => getMaterialType(m) === 'audio'))}
                                                        {renderMaterialSlider('pdf', 'PDF Documents', studyMaterials.filter(m => getMaterialType(m) === 'pdf'))}
                                                        {renderMaterialSlider('web', 'Web Links / Pages', studyMaterials.filter(m => getMaterialType(m) === 'web'))}
                                                        ...
                                                        {studyMaterials.length === 0 && (
                                                            <div className="py-12 text-center bg-white rounded-2xl border border-slate-100 shadow-sm max-w-md mx-auto">
                                                                ...
                                                                <p className="text-slate-455 text-xs mt-1 font-medium">No materials uploaded yet.</p>
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    renderMaterialSlider(selectedCategoryTab, `${selectedCategoryTab === 'web' ? 'Web page' : selectedCategoryTab}s`, filteredMaterials)
                                                )}
                                            </div>
                                        </div>
                                    ) : viewMode === 'tools' ? (
                        ...
                        <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl border-none flex flex-col max-h-[85vh] overflow-hidden relative animate-slide-up animate-fade-in">
                            ...
                            <div className="px-8 py-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar text-left">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div
                                        className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${isNoExpiry
                                    ...
                                    <div
                                        className={`p-4 rounded-2xl border transition-all flex flex-col gap-3 ${!isNoExpiry
                                ...
                                <div className="space-y-4 pt-4">
                                    ...
                                    {selectedStudent && (
                                        <div
                                            className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${assignmentScope === 'particular'
                                                    ? 'bg-indigo-50/40 border-indigo-200'
                                                    : 'bg-white border-slate-200 hover:border-slate-300'
                                                }`}
                                    ...
                                    {/* Selected Students */}
                                    <div
                                        className={`p-4 rounded-2xl border transition-all flex flex-col gap-3 ${assignmentScope === 'selected'
                                                ? 'bg-indigo-50/40 border-indigo-200'
                                                : 'bg-white border-slate-200 hover:border-slate-300'
                                            }`}
                                    >
                                    ...
                                    {/* All Students */}
                                    <div
                                        className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${assignmentScope === 'all'
                                                ? 'bg-indigo-50/40 border-indigo-200'
                                                : 'bg-white border-slate-200 hover:border-slate-300'
                                            }`}
